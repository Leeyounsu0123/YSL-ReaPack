...
--   - js_ReaScriptAPI is optional and enables direct Media Explorer search-history capture
--   ## License
...
-- A second invocation from the Action List terminates the running deferred
-- instance and does not open a duplicate window.
if reaper.set_action_options then
    pcall(reaper.set_action_options, 1)
end
...
local function ImGuiResolve(value)
...
local function ImGuiConst(value, fallback)
    local resolved = ImGuiResolve(value)
    if resolved ~= nil then return resolved end
    if fallback ~= nil then return fallback end
    return 0
end

local IMGUI_CONFIG_DOCKING_ENABLE = ImGuiConst(ImGui.ConfigFlags_DockingEnable, 0)
ImGui.ChildFlags_Border = ImGuiResolve(ImGui.ChildFlags_Border or ImGui.ChildFlags_Borders)
    or true
ImGui.WindowFlags_AlwaysAutoResize = ImGuiConst(ImGui.WindowFlags_AlwaysAutoResize, 0)
ImGui.InputTextFlags_EnterReturnsTrue = ImGuiConst(ImGui.InputTextFlags_EnterReturnsTrue, 0)
ImGui.ColorEditFlags_NoInputs = ImGuiConst(ImGui.ColorEditFlags_NoInputs, 0)
...
local BOTTOM_NOTICE_H = 68
local MEDIA_EXPLORER_HISTORY_LIMIT = 30
local MEDIA_EXPLORER_SEARCH_CONTROL_ID = 1015

local context_flags = IMGUI_CONFIG_DOCKING_ENABLE
local ctx = ImGui.CreateContext(APP_NAME, context_flags)
local ui_font = ImGui.CreateFont and ImGui.CreateFont('sans-serif', 12) or nil
if ui_font and ImGui.Attach then pcall(ImGui.Attach, ctx, ui_font) end

local function ImGuiIsKeyPressed(key)
    key = ImGuiResolve(key)
    if key == nil or not ImGui.IsKeyPressed then return false end
    local ok, pressed = pcall(ImGui.IsKeyPressed, ctx, key)
    return ok and pressed == true
end

YSL_SLM_ACTION_TOGGLE = { bindings = {}, armed = false }

YSL_SLM_ACTION_TOGGLE.key_from_vk = function(vk)
    local key_code = tonumber(vk) or -1
    if key_code >= 0x8000 then key_code = key_code & 0xFF end

    local suffix
    if key_code >= 48 and key_code <= 57 then
        suffix = string.char(key_code)
    elseif key_code >= 65 and key_code <= 90 then
        suffix = string.char(key_code)
    elseif key_code >= 112 and key_code <= 135 then
        suffix = 'F' .. tostring(key_code - 111)
    elseif key_code >= 96 and key_code <= 105 then
        suffix = 'Keypad' .. tostring(key_code - 96)
    else
        local names = {
            ...
            [106] = 'KeypadMultiply', [107] = 'KeypadAdd', [109] = 'KeypadSubtract',
            [110] = 'KeypadDecimal', [111] = 'KeypadDivide',
            [144] = 'NumLock', [145] = 'ScrollLock',
            [186] = 'Semicolon', [187] = 'Equal', [188] = 'Comma', [189] = 'Minus',
            [190] = 'Period', [191] = 'Slash', [192] = 'GraveAccent',
            [219] = 'LeftBracket', [220] = 'Backslash', [221] = 'RightBracket',
            [222] = 'Apostrophe',
        }
        suffix = names[key_code]
    end

    return suffix and ImGuiResolve(ImGui['Key_' .. suffix]) or nil
end

YSL_SLM_ACTION_TOGGLE.load_bindings = function()
    local _, _, section_id, command_id = reaper.get_action_context()
    local named = reaper.ReverseNamedCommandLookup and
        reaper.ReverseNamedCommandLookup(command_id) or ''
    if not named or named == '' then return end

    local token = '_' .. named
    local separator = package.config:sub(1, 1)
    local file = io.open(reaper.GetResourcePath() .. separator .. 'reaper-kb.ini', 'r')
    if not file then return end

    for line in file:lines() do
        local flags, vk, command, section =
            line:match('^KEY%s+(%d+)%s+(%d+)%s+(%S+)%s+(%-?%d+)')
        if command == token and tonumber(section) == section_id then
            local key = YSL_SLM_ACTION_TOGGLE.key_from_vk(vk)
            if key then
                YSL_SLM_ACTION_TOGGLE.bindings[#YSL_SLM_ACTION_TOGGLE.bindings + 1] = {
                    flags = tonumber(flags) or 1,
                    key = key,
                }
            end
        end
...
YSL_SLM_ACTION_TOGGLE.modifiers_match = function(flags, mods)
    local shift = ImGuiResolve(ImGui.Mod_Shift) or 0
    local ctrl = ImGuiResolve(ImGui.Mod_Ctrl) or 0
    local alt = ImGuiResolve(ImGui.Mod_Alt) or 0
    local super = ImGuiResolve(ImGui.Mod_Super) or 0
    return ((flags & 4) ~= 0) == ((mods & shift) ~= 0)
        and ((flags & 8) ~= 0) == ((mods & ctrl) ~= 0)
        and ((flags & 16) ~= 0) == ((mods & alt) ~= 0)
        and ((flags & 32) ~= 0) == ((mods & super) ~= 0)
end

YSL_SLM_ACTION_TOGGLE.pressed = function()
    if #YSL_SLM_ACTION_TOGGLE.bindings == 0 or
        not ImGui.IsKeyDown or not ImGui.IsKeyPressed or
        not ImGui.GetKeyMods then return false end

    local any_down = false
    for _, binding in ipairs(YSL_SLM_ACTION_TOGGLE.bindings) do
        local ok, down = pcall(ImGui.IsKeyDown, ctx, binding.key)
        if ok and down then any_down = true; break end
    end

    -- Do not mistake the launch shortcut for a close request.
    if not YSL_SLM_ACTION_TOGGLE.armed then
        if not any_down then YSL_SLM_ACTION_TOGGLE.armed = true end
        return false
    end

    local ok_mods, mods = pcall(ImGui.GetKeyMods, ctx)
    if not ok_mods then return false end
    for _, binding in ipairs(YSL_SLM_ACTION_TOGGLE.bindings) do
        if YSL_SLM_ACTION_TOGGLE.modifiers_match(binding.flags, mods) then
            local ok, was_pressed = pcall(ImGui.IsKeyPressed, ctx, binding.key, false)
            if ok and was_pressed then return true end
        end
    end
    return false
end

YSL_SLM_ACTION_TOGGLE.load_bindings()

local function ImGuiSetNextWindowPosSafe(x, y, condition)
    if not ImGui.SetNextWindowPos then return end
    if x == nil or y == nil then return end
    pcall(ImGui.SetNextWindowPos, ctx, x, y, ImGuiConst(condition, 0))
end

local function ImGuiSetNextWindowSizeSafe(width, height, condition)
    if not ImGui.SetNextWindowSize then return end
    width = math.max(320, tonumber(width) or DEFAULT_WINDOW_W)
    height = math.max(360, tonumber(height) or DEFAULT_WINDOW_H)
    pcall(ImGui.SetNextWindowSize, ctx, width, height, ImGuiConst(condition, 0))
end
...
local media_explorer_history = {}
local media_explorer_auto_record = true
local media_explorer_monitor = {
    window = nil,
    edit = nil,
    last_text = '',
    pending_text = '',
...
local last_deleted_media_history = nil
...
local stored_window_h = tonumber(reaper.GetExtState(EXT_SECTION, 'WindowH')) or DEFAULT_WINDOW_H
local window_state = {
    x = tonumber(reaper.GetExtState(EXT_SECTION, 'WindowX')),
    y = tonumber(reaper.GetExtState(EXT_SECTION, 'WindowY')),
    w = stored_window_geometry_version < WINDOW_GEOMETRY_VERSION and DEFAULT_WINDOW_W
        or math.max(stored_window_w, DEFAULT_WINDOW_W),
    h = stored_window_geometry_version < WINDOW_GEOMETRY_VERSION and math.max(stored_window_h, DEFAULT_WINDOW_H)
        or math.max(stored_window_h, 560),
}
...
local function DrawStatus()
    if status_message ~= '' and reaper.time_precise() < status_until then
        if ImGui.PushTextWrapPos then
            ...
            ImGui.PushTextWrapPos(ctx, ImGui.GetCursorPosX(ctx) + math.max(1, available_width))
        end
...
local function ExportDiagnosticLog(reason)
    ...
    local lines = {
        ...
        string.format('State: keywords=%d tags=%d keyword_undo=%s history_undo=%s revision=%d', #keywords, #tags, last_deleted_keyword and 'active' or 'none', last_deleted_media_history and 'active' or 'none', data_revision),
        '', 'Events:'
    }
...
local function SaveWindowState(force)
    ...
    -- A dock node reports transient dimensions while a tab is being detached.
    -- Saving during that moment can restore an invalid floating rectangle later.
...
local function UpdateDockTransitionState()
    local docked = false
    if ImGui.IsWindowDocked then
        local ok, value = pcall(ImGui.IsWindowDocked, ctx)
...
SLM_UI_SELECTED_BUTTON = 0x2F6F9FFF
SLM_UI_SELECTED_BUTTON_HOVERED = 0x3B86BCFF
SLM_UI_SELECTED_BUTTON_ACTIVE = 0x285D85FF

function SLM_DrawToggleButton(label, id, selected, width, height)
    label = tostring(label or '')
    if selected then
        ImGui.PushStyleColor(ctx, ImGui.Col_Button, SLM_UI_SELECTED_BUTTON)
        ImGui.PushStyleColor(ctx, ImGui.Col_ButtonHovered, SLM_UI_SELECTED_BUTTON_HOVERED)
        ImGui.PushStyleColor(ctx, ImGui.Col_ButtonActive, SLM_UI_SELECTED_BUTTON_ACTIVE)
...
local function DrawTagButton(label, color, id, width, height, selected)
    label = tostring(label or '')
    local source_color = MakeOpaque(color)
    local base_color = selected == false and AdjustColor(source_color, 0.55) or source_color
    local hover_color = AdjustColor(base_color, selected == true and 1.16 or 1.12)
...
local function DrawWrappedTagButtons(tag_items, id_prefix, on_click, show_count, selected_set)
    ...
    for index, tag_item in ipairs(tag_items) do
        ...
        if tag_name then
            ...
            local selection_state = selected_set and selected or nil
            if DrawTagButton(label, color, button_id, width, 21, selection_state) and on_click then
                on_click(tag_name)
            end
        end
    end
...
local function BuildDataTable()
    return {
        ...
        tags = tags,
        keywords = keywords,
        media_explorer_history = media_explorer_history,
        settings = {
            filter_mode = filter_mode,
            sort_mode = sort_mode,
            favorite_only = favorite_only,
            smart_collection = smart_collection,
            media_explorer_auto_record = media_explorer_auto_record,
        },
    }
end
...
local function ApplyDataTable(data)
    ...
    media_explorer_history = {}
    local history_seen = {}
    for _, raw_entry in ipairs(type(data.media_explorer_history) == 'table' and data.media_explorer_history or {}) do
        local query, last_used, use_count
        if type(raw_entry) == 'table' then
            query = SanitizeStoredText(raw_entry.query)
            last_used = math.max(0, math.floor(tonumber(raw_entry.last_used) or 0))
            use_count = math.max(1, math.floor(tonumber(raw_entry.use_count) or 1))
        else
            query = SanitizeStoredText(raw_entry)
            last_used = 0
            use_count = 1
        end
        local key = Lower(query)
        if query ~= '' and not history_seen[key] then
            history_seen[key] = true
            media_explorer_history[#media_explorer_history + 1] = {
                query = query,
                last_used = last_used,
                use_count = use_count,
            }
    ...
    table.sort(media_explorer_history, function(a, b)
        if a.last_used == b.last_used then return Lower(a.query) < Lower(b.query) end
        return a.last_used > b.last_used
    end)
    while #media_explorer_history > MEDIA_EXPLORER_HISTORY_LIMIT do
        table.remove(media_explorer_history)
    end
    ...
    media_explorer_auto_record = settings.media_explorer_auto_record ~= false
...
local function UndoLastKeywordDelete()
    ...
    if keyword.id >= next_keyword_id then next_keyword_id = keyword.id + 1 end
...
function SLM_UndoLastMediaHistoryDelete()
    local deleted = last_deleted_media_history
    if not deleted then return false end
    if reaper.time_precise() > (deleted.expires_at or 0) then
        last_deleted_media_history = nil
        SetStatus(L("검색 기록 실행 취소 시간이 지났습니다.", "The history undo period has expired."))
        return false
    end

    local merged, seen = {}, {}
    for _, entry in ipairs(media_explorer_history) do
        local key = Lower(entry.query)
        if key ~= '' and not seen[key] then
            seen[key] = true
            merged[#merged + 1] = entry
        end
    end
    for _, entry in ipairs(deleted.entries or {}) do
        local key = Lower(entry.query)
        if key ~= '' and not seen[key] then
            seen[key] = true
            merged[#merged + 1] = {
                ...
                use_count = math.max(1, tonumber(entry.use_count) or 1),
            }
        end
    end
    ...
    while #merged > MEDIA_EXPLORER_HISTORY_LIMIT do table.remove(merged) end
    media_explorer_history = merged
    last_deleted_media_history = nil
    ScheduleSave(false, 0.1)
    SetStatus(L("삭제한 Media Explorer 검색 기록을 복구했습니다.", "Restored deleted Media Explorer history."), 3.0)
    return true
end

local function DrawUndoDelete()
    local drew = false
    if last_deleted_keyword then
        local remaining = (last_deleted_keyword.expires_at or 0) - reaper.time_precise()
        if remaining <= 0 then
            last_deleted_keyword = nil
        else
            drew = true
            ImGui.TextColored(ctx, 0xFFD166FF,
                string.format(L("삭제한 검색어를 %.0f초 동안 복구할 수 있습니다.", "Deleted keyword can be restored for %.0f seconds."), math.ceil(remaining)))
            ImGui.SameLine(ctx)
            if ImGui.Button(ctx, L("실행 취소", "Undo") .. "##UndoKeywordDelete", 72, 20) then UndoLastKeywordDelete() end
        end
    end

    if last_deleted_media_history then
        local remaining = (last_deleted_media_history.expires_at or 0) - reaper.time_precise()
        if remaining <= 0 then
            last_deleted_media_history = nil
        else
            drew = true
            ImGui.TextColored(ctx, 0xFFD166FF,
                string.format(L("삭제한 검색 기록을 %.0f초 동안 복구할 수 있습니다.", "Deleted history can be restored for %.0f seconds."), math.ceil(remaining)))
            ImGui.SameLine(ctx)
            if ImGui.Button(ctx, L("실행 취소", "Undo") .. "##UndoMediaHistoryDelete", 72, 20) then SLM_UndoLastMediaHistoryDelete() end
        end
    end
    return drew
end

local function DrawBottomNotifications()
    local footer_visible = ImGui.BeginChild(
        ctx, 'BottomNotifications', 0, BOTTOM_NOTICE_H, ImGui.ChildFlags_Border)
    if footer_visible then
        DrawStatus()
        DrawUndoDelete()
        ImGui.EndChild(ctx)
    end
...
-- -----------------------------------------------------------------------------
local function RecordMediaExplorerHistory(query)
    query = SanitizeStoredText(query)
    if query == '' then return false end

    local normalized = Lower(query)
    local entry = nil
    for index = #media_explorer_history, 1, -1 do
        local candidate = media_explorer_history[index]
        if Lower(candidate.query) == normalized then
            entry = candidate
            table.remove(media_explorer_history, index)
            break
        end
    end
    if not entry then entry = {query = query, last_used = 0, use_count = 0} end
    entry.query = query
    entry.last_used = os.time()
    entry.use_count = math.max(0, tonumber(entry.use_count) or 0) + 1
    table.insert(media_explorer_history, 1, entry)
    while #media_explorer_history > MEDIA_EXPLORER_HISTORY_LIMIT do table.remove(media_explorer_history) end
    ScheduleSave(false, 0.4)
    return true
end

local function MediaExplorerSearchEdit(open_if_needed)
    if not reaper.JS_Window_FindChildByID or not reaper.JS_Window_GetTitle then return nil end

    local function valid(hwnd)
        if not hwnd then return false end
        if reaper.JS_Window_IsWindow then
            local ok, result = pcall(reaper.JS_Window_IsWindow, hwnd)
    ...
    local hwnd = nil
    if open_if_needed and reaper.OpenMediaExplorer then
        local ok, result = pcall(reaper.OpenMediaExplorer, '', false)
        if ok and valid(result) then hwnd = result end
    end

    if not valid(hwnd) and valid(media_explorer_monitor.window) and valid(media_explorer_monitor.edit) then
        return media_explorer_monitor.edit, media_explorer_monitor.window
    end
    media_explorer_monitor.window, media_explorer_monitor.edit = nil, nil

    local toggle = reaper.GetToggleCommandState and reaper.GetToggleCommandState(50124) or -1
    if not valid(hwnd) and reaper.OpenMediaExplorer and toggle > 0 then
        local ok, result = pcall(reaper.OpenMediaExplorer, '', false)
        if ok and valid(result) then hwnd = result end
    end
    if not valid(hwnd) and reaper.JS_Window_Find then
        local title = reaper.JS_Localize and reaper.JS_Localize('Media Explorer', 'common') or 'Media Explorer'
        local ok, result = pcall(reaper.JS_Window_Find, title, true)
        if ok then hwnd = result end
    end
    if not valid(hwnd) then return nil end

    local ok, edit = pcall(reaper.JS_Window_FindChildByID, hwnd, MEDIA_EXPLORER_SEARCH_CONTROL_ID)
    if not ok or not valid(edit) then return nil end
    media_explorer_monitor.window, media_explorer_monitor.edit = hwnd, edit
    return edit, hwnd
end

function SLM_GetCurrentMediaExplorerQuery()
    local edit = MediaExplorerSearchEdit()
    if not edit or not reaper.JS_Window_GetTitle then return '' end
    local ok, value = pcall(reaper.JS_Window_GetTitle, edit)
    if not ok then return '' end
    return SanitizeStoredText(value)
end

function SLM_ApplyMediaExplorerQuery(query)
    local edit, window = MediaExplorerSearchEdit(true)
    if not edit or not reaper.JS_Window_SetTitle then return false end

    local set_ok, set_result = pcall(reaper.JS_Window_SetTitle, edit, query)
    if not set_ok or set_result == false then return false end

    -- Setting a combo-box title changes its text but does not reliably refresh
    -- Media Explorer results. Send the same CBN_EDITCHANGE notification that
    -- REAPER receives when the user types into control 1015.
    local notified = false
    if reaper.JS_WindowMessage_Send then
        local parent = window
        if reaper.JS_Window_GetParent then
            local parent_ok, result = pcall(reaper.JS_Window_GetParent, edit)
            if parent_ok and result then parent = result end
        end
        if parent then
            local address = 0
            if reaper.JS_Window_AddressFromHandle then
                local address_ok, result = pcall(reaper.JS_Window_AddressFromHandle, edit)
                if address_ok and result then address = result end
            end
            local send_ok, send_result = pcall(
                reaper.JS_WindowMessage_Send,
                parent,
                'WM_COMMAND',
                MEDIA_EXPLORER_SEARCH_CONTROL_ID,
                5,
                address,
                0
            )
            notified = send_ok and send_result ~= false
        end
    end

    -- SWS uses the Media Explorer root directly and covers older js_API builds
    -- whose WM_COMMAND forwarding is incomplete.
    if reaper.BR_Win32_SendMessage and window then
        local sws_ok, sws_result = pcall(
            reaper.BR_Win32_SendMessage,
            window,
            0x0111,
            (5 << 16) | MEDIA_EXPLORER_SEARCH_CONTROL_ID,
            0
        )
        notified = notified or (sws_ok and sws_result ~= false)
    end

    if not notified then return false end

    media_explorer_monitor.last_text = query
    media_explorer_monitor.pending_text = query
    media_explorer_monitor.last_recorded_text = query
    return true
end

function SLM_ProcessPendingMediaExplorerSend()
    local query = media_explorer_monitor.send_query
    if not query then return end

    local now = reaper.time_precise()
    if now < media_explorer_monitor.send_next_try then return end

    if SLM_ApplyMediaExplorerQuery(query) then
        media_explorer_monitor.send_query = nil
        SetStatus(L('Media Explorer 검색을 적용했습니다: ', 'Applied the Media Explorer search: ') .. query, 4.0)
        return
    end

    if now >= media_explorer_monitor.send_deadline then
        media_explorer_monitor.send_query = nil
        CopyText(query, nil)
        SetStatus(L('Media Explorer 검색 적용에 실패해 검색어를 복사했습니다: ', 'Media Explorer search failed, so the query was copied: ') .. query, 5.0)
        return
    end

    media_explorer_monitor.send_next_try = now + 0.08
end

function SLM_SaveQueryToLibrary(query, open_editor)
    local word = SanitizeStoredText(query)
    if word == '' then
        SetStatus(L('저장할 검색어가 없습니다.', 'There is no query to save.'), 3.0)
        return nil, false
    end

    local keyword = FindDuplicateKeyword(word, nil)
    local created = false
    if not keyword then
        keyword = {
            id = next_keyword_id,
        ...
        created = true
        SaveData({save_legacy = true, invalidate_tag_cache = false})
    end

    if open_editor and type(SLM_BeginKeywordEdit) == 'function' then SLM_BeginKeywordEdit(keyword) end
    SetStatus((created and L('검색어를 저장했습니다: ', 'Saved keyword: ') or L('저장된 검색어를 편집합니다: ', 'Editing saved keyword: ')) .. word, 3.0)
    return keyword, created
end

function SLM_CopyMediaHistoryEntries(entries)
    local result = {}
    for _, entry in ipairs(entries or {}) do
        result[#result + 1] = {
            query = tostring(entry.query or ''),
            last_used = tonumber(entry.last_used) or 0,
            use_count = math.max(1, tonumber(entry.use_count) or 1),
        }
    end
...
function SLM_DeleteMediaExplorerHistoryEntry(index)
    local entry = media_explorer_history[index]
    if not entry then return false end
    local answer = reaper.ShowMessageBox(
        string.format(L("검색 기록 '%s'을(를) 삭제하시겠습니까?", "Delete history '%s'?"), entry.query),
        L('검색 기록 삭제', 'Delete Search History'), 4)
    if answer ~= 6 then return false end

    last_deleted_media_history = {
        entries = SLM_CopyMediaHistoryEntries({entry}),
        expires_at = reaper.time_precise() + UNDO_DELETE_SECONDS,
    }
    table.remove(media_explorer_history, index)
    ScheduleSave(false, 0.1)
    SetStatus(L('검색 기록을 삭제했습니다. 8초 동안 실행 취소할 수 있습니다.', 'Search history deleted. You can undo for 8 seconds.'), UNDO_DELETE_SECONDS)
    return true
end

function SLM_ClearMediaExplorerHistory()
    if #media_explorer_history == 0 then return false end
    local answer = reaper.ShowMessageBox(
        string.format(L('Media Explorer 검색 기록 %d개를 모두 삭제하시겠습니까?', 'Delete all %d Media Explorer history entries?'), #media_explorer_history),
        L('검색 기록 전체 삭제', 'Clear Search History'), 4)
    if answer ~= 6 then return false end

    last_deleted_media_history = {
        entries = SLM_CopyMediaHistoryEntries(media_explorer_history),
        expires_at = reaper.time_precise() + UNDO_DELETE_SECONDS,
    }
    media_explorer_history = {}
    media_explorer_monitor.last_recorded_text = ''
    ScheduleSave(false, 0.1)
    SetStatus(L('검색 기록을 모두 삭제했습니다. 8초 동안 실행 취소할 수 있습니다.', 'All search history was deleted. You can undo for 8 seconds.'), UNDO_DELETE_SECONDS)
    return true
end

local function PollMediaExplorerSearchHistory()
    ...
    local edit = MediaExplorerSearchEdit()
    if not edit then
        media_explorer_monitor.last_text = ''
        media_explorer_monitor.pending_text = ''
        return
    end

    local ok, value = pcall(reaper.JS_Window_GetTitle, edit)
    if not ok then return end
    local query = SanitizeStoredText(value)
    if query ~= media_explorer_monitor.last_text then
        media_explorer_monitor.last_text = query
        media_explorer_monitor.pending_text = query
        media_explorer_monitor.changed_at = now
        if query == '' then media_explorer_monitor.last_recorded_text = '' end
        return
    end

    if query ~= '' and query == media_explorer_monitor.pending_text
        and query ~= media_explorer_monitor.last_recorded_text
        and now - media_explorer_monitor.changed_at >= 0.9 then
        if RecordMediaExplorerHistory(query) then media_explorer_monitor.last_recorded_text = query end
    end
end

local function DrawMediaExplorerHistory()
    ImGui.Spacing(ctx)
    ImGui.TextColored(ctx, UI_ACCENT, L('Media Explorer 검색 기록', 'Media Explorer Search History'))
    ImGui.SameLine(ctx, 0, 8)
    ImGui.TextDisabled(ctx, media_explorer_auto_record
        and L('직접 입력한 검색어 자동 기록 중', 'Automatically capturing typed searches')
        or L('자동 기록 꺼짐', 'Automatic capture is off'))

    if SLM_DrawToggleButton(L('자동 기록', 'Auto Record'), 'MediaHistoryAutoRecord', media_explorer_auto_record, 88, 21) then
        media_explorer_auto_record = not media_explorer_auto_record
        media_explorer_monitor.last_text = ''
        media_explorer_monitor.pending_text = ''
        media_explorer_monitor.last_recorded_text = ''
        ScheduleSave(false, 0.1)
        SetStatus(media_explorer_auto_record and L('Media Explorer 검색어 자동 기록을 켰습니다.', 'Media Explorer automatic history is on.')
            or L('Media Explorer 검색어 자동 기록을 껐습니다.', 'Media Explorer automatic history is off.'), 3.0)
    end
    ImGui.SameLine(ctx, 0, 6)
    if ImGui.Button(ctx, L('현재 검색어 저장', 'Save Current Query') .. '##SaveCurrentMediaQuery', 112, 21) then
        local current_query = SLM_GetCurrentMediaExplorerQuery()
        if current_query == '' then
            SetStatus(L('Media Explorer 검색창에서 저장할 검색어를 찾지 못했습니다.', 'No query was found in the Media Explorer search field.'), 4.0)
        else
            SLM_SaveQueryToLibrary(current_query, true)
        end
    end
    ImGui.SameLine(ctx, 0, 6)
    if ImGui.Button(ctx, L('기록 전체 삭제', 'Clear History') .. '##ClearMediaHistory', 96, 21) then SLM_ClearMediaExplorerHistory() end

    local history_visible = ImGui.BeginChild(ctx, 'MediaExplorerSearchHistory', 0, 184, ImGui.ChildFlags_Border)
    if history_visible then
        if #media_explorer_history == 0 then
            if media_explorer_auto_record and not reaper.JS_Window_FindChildByID then
                ImGui.TextDisabled(ctx, L('직접 검색 기록 감지에는 js_ReaScriptAPI가 필요합니다.', 'js_ReaScriptAPI is required to capture direct searches.'))
            else
                ImGui.TextDisabled(ctx, L('아직 기록이 없습니다.', 'No history yet.'))
            end
        else
            for index = 1, math.min(#media_explorer_history, 12) do
                local entry = media_explorer_history[index]
                local existing = FindDuplicateKeyword(entry.query, nil)
                local available = ImGui.GetContentRegionAvail(ctx)
                local spacing = 5
                local save_width, search_width, delete_width = 48, 48, 48
                local action_width = save_width + search_width + delete_width + spacing * 3
                local query_width = math.max(120, available - action_width)

                if ImGui.Button(ctx, entry.query .. '##MediaHistory' .. tostring(index), query_width, 22) then
                    search_text = entry.query
                    InvalidateVisibleKeywordCache()
                    SetStatus(L('검색 기록을 빠른 검색칸에 불러왔습니다: ', 'Loaded history into Quick Search: ') .. entry.query, 2.5)
                end
                if ImGui.IsItemHovered(ctx) then ImGui.SetTooltip(ctx, L('클릭하면 빠른 검색칸에 불러옵니다.', 'Click to load into Quick Search.')) end

                ImGui.SameLine(ctx, 0, spacing)
                if ImGui.Button(ctx, (existing and L('편집', 'Edit') or L('저장', 'Save')) .. '##HistorySaveEdit' .. tostring(index), save_width, 22) then
                    SLM_SaveQueryToLibrary(entry.query, true)
                    break
                end
                ImGui.SameLine(ctx, 0, spacing)
                if ImGui.Button(ctx, L('검색', 'Search') .. '##HistorySearch' .. tostring(index), search_width, 22) then
                    SLM_SendToMediaExplorer(entry.query, {})
                    break
                end
                ImGui.SameLine(ctx, 0, spacing)
                if DrawDangerButton(L('삭제', 'Delete'), 'HistoryDelete' .. tostring(index), delete_width, 22) then
                    if SLM_DeleteMediaExplorerHistoryEntry(index) then break end
                end
                ImGui.TextDisabled(ctx, string.format('%s · %d', FormatLastUsed(entry.last_used), entry.use_count or 1))
                ImGui.Separator(ctx)
            end
        end
        ImGui.EndChild(ctx)
...
local function DrawTagSuggestions(text_value, selected_set, toggle_callback, id_prefix)
    ...
    ImGui.TextDisabled(ctx, L('추천 태그 · 클릭하면 적용', 'Suggested tags · Click to apply'))
    ImGui.SameLine(ctx, 0, 8)
...
function SLM_SendToMediaExplorer(query, used_keywords)
    query = CollapseSpaces(query)
    if query == '' then
        SetStatus(L('Media Explorer로 보낼 검색어가 없습니다.', 'There is no query to send to Media Explorer.'))
        return
    end

    RecordMediaExplorerHistory(query)
    media_explorer_monitor.last_recorded_text = query

    local touched_keywords = {}
    local usage_changed = false
    local function touch_keyword(keyword)
        if not keyword or touched_keywords[keyword] then return end
        touched_keywords[keyword] = true
        keyword.use_count = (tonumber(keyword.use_count) or 0) + 1
        keyword.last_used = os.time()
        usage_changed = true
    end

    local saved_keyword = FindDuplicateKeyword(query, nil)
    touch_keyword(saved_keyword)
    if used_keywords then
        for _, keyword in ipairs(used_keywords) do touch_keyword(keyword) end
    end
    if usage_changed then
        InvalidateVisibleKeywordCache()
        ScheduleSave(false, 0.3)
    end

    if not reaper.JS_Window_FindChildByID or not reaper.JS_Window_SetTitle then
        if reaper.OpenMediaExplorer then pcall(reaper.OpenMediaExplorer, '', false)
        elseif reaper.Main_OnCommand then reaper.Main_OnCommand(50124, 0) end
        CopyText(query, nil)
        SetStatus(L('직접 검색에는 js_ReaScriptAPI가 필요해 검색어를 복사했습니다: ', 'Direct search needs js_ReaScriptAPI, so the query was copied: ') .. query, 5.0)
        return
    end

    media_explorer_monitor.send_query = query
    media_explorer_monitor.send_deadline = reaper.time_precise() + 1.2
    media_explorer_monitor.send_next_try = 0
    if not reaper.OpenMediaExplorer and reaper.Main_OnCommand then
        reaper.Main_OnCommand(50124, 0)
    end
    SLM_ProcessPendingMediaExplorerSend()
    if media_explorer_monitor.send_query then
        SetStatus(L('Media Explorer를 열고 검색을 적용하는 중입니다...', 'Opening Media Explorer and applying the search...'), 2.0)
    end
end
...
local function DrawSmartCollectionControl()
    ...
    if ImGui.BeginCombo(ctx, L('스마트 컬렉션', 'Smart Collection') .. '##SmartCollection', labels[smart_collection] or labels.all) then
        ...
        for _, key in ipairs(order) do
            if ImGui.Selectable(ctx, labels[key], smart_collection == key) then
                smart_collection = key
                local collection_sort = {
                    all='newest', favorites='favorite', recent7='recent',
                    popular='usage', unused='name', stale30='recent'
                }
                sort_mode = collection_sort[key] or 'newest'
                InvalidateVisibleKeywordCache()
                ScheduleSave(false,0.3)
            end
        end
...
function SLM_SaveOrTouchSearchQuery(query)
    local word = SanitizeStoredText(query)
    if word == '' then return nil, false, false end
    ...
    local saved = SaveData({save_legacy = true, invalidate_tag_cache = false})
    return keyword, created, saved == true
end
...
local function SaveNewKeyword()
    ...
    local tag_summary = #assigned_tags > 0
        and (L(" · 적용 태그: ", " · Tags: ") .. table.concat(assigned_tags, ', '))
        or L(" · 적용 태그 없음", " · No tags")
    SetStatus(L("검색어를 저장했습니다: ", "Keyword saved: ") .. word .. tag_summary, 4.0)
    return true
end
...
function SLM_BeginKeywordEdit(keyword)
...
local function DrawKeywordTagArea(keyword, keyword_id, area_width)
    local keyword_tags = keyword.tags or {}
    if #keyword_tags == 0 then
        return false
    end
    area_width = math.max(1, tonumber(area_width) or 1)
    ...
    if keyword_tags_visible then
        ...
        for tag_index, tag_name in ipairs(keyword_tags) do
    ...
    return keyword_tags_visible == true
end

local function DrawKeywordActions(keyword, keyword_index)
    local spacing = 4
    local edit_width = 44
    local media_width = 52
    local delete_width = 44
    if ImGui.Button(ctx, L("편집##Edit", "Edit##Edit") .. tostring(keyword.id), edit_width, 19) then
        SLM_BeginKeywordEdit(keyword)
    end
    ImGui.SameLine(ctx, 0, spacing)
    if ImGui.Button(ctx, L("검색##MediaSearch", "Search##MediaSearch") .. tostring(keyword.id), media_width, 19) then
        SLM_SendToMediaExplorer(keyword.word, {keyword})
    end
    if ImGui.IsItemHovered(ctx) then
        ImGui.SetTooltip(ctx, L("Media Explorer에서 바로 검색", "Search directly in Media Explorer"))
    end
    ImGui.SameLine(ctx, 0, spacing)
...
local function DrawKeywordSecondRow(keyword)
    local available_width = math.max(1, ImGui.GetContentRegionAvail(ctx))
    return DrawKeywordTagArea(keyword, keyword.id, available_width)
end
...
local function DrawKeywordRow(keyword, keyword_index)
    ...
    local action_width = 44 + 52 + 44 + 4 * 2
    local action_gap = 8
    local text_x = ImGui.GetCursorPosX(ctx)
    local text_y = ImGui.GetCursorPosY(ctx)
    local row_width = ImGui.GetContentRegionAvail(ctx)
    local text_width = math.max(80, row_width - action_width - action_gap)
    local action_x = text_x + math.max(0, row_width - action_width)
    if ImGui.PushTextWrapPos then
        ImGui.PushTextWrapPos(ctx, text_x + text_width)
    end
    ImGui.TextWrapped(ctx, keyword.word)
    if ImGui.PopTextWrapPos then
        ImGui.PopTextWrapPos(ctx)
    end
    local after_text_y = ImGui.GetCursorPosY(ctx)
    if ImGui.IsItemHovered(ctx) then
        ImGui.SetTooltip(
            ctx,
            L("더블클릭: Media Explorer에서 검색\n최근 사용: ", "Double-click: Search in Media Explorer\nLast used: ") .. FormatLastUsed(keyword.last_used)
    ...
    ImGui.SetCursorPosX(ctx, action_x)
    local deleted = DrawKeywordActions(keyword, keyword_index)
    local after_actions_y = ImGui.GetCursorPosY(ctx)
    ImGui.SetCursorPosY(ctx, math.max(after_text_y, after_actions_y))
    DrawKeywordSecondRow(keyword)
...
local function DrawFilterAndSearchControls()
    ...
    ImGui.SetNextItemWidth(ctx, -274)
    ...
    ImGui.SameLine(ctx, 0, 6)
    if ImGui.Button(ctx, L("Media 검색", "Media Search"), 96, 0) then SLM_SendToMediaExplorer(search_text, {}) end
    if ImGui.IsItemHovered(ctx) then
        ImGui.SetTooltip(ctx, L("현재 문구를 Media Explorer로 보냅니다. 새 검색어로 자동 저장하지 않습니다.", "Sends the text to Media Explorer without automatically saving a new keyword."))
    end

    ImGui.SameLine(ctx, 0, 6)
    if ImGui.Button(ctx, L("검색어 저장", "Save Keyword"), 86, 0) then SLM_SaveQueryToLibrary(search_text, true) end
    ImGui.SameLine(ctx, 0, 6)
    if ImGui.Button(ctx, L("지우기", "Clear"), 58, 0) then search_text = '' end

    ImGui.TextDisabled(ctx, L("빠른 검색은 목록 필터 전용입니다. Media 검색과 검색어 저장은 오른쪽 버튼으로 실행합니다.", "Quick Search only filters the list. Use the buttons to search Media Explorer or save a keyword."))
    ...
    if filter_panel_open then
        ...
        ImGui.SameLine(ctx, 0, 8)
        if SLM_DrawToggleButton('AND', 'FilterModeAND', filter_mode == 'AND', 52, 21) then
            filter_mode = 'AND'
            ScheduleSave(false, 0.5)
        end
        ImGui.SameLine(ctx, 0, 4)
        if SLM_DrawToggleButton('OR', 'FilterModeOR', filter_mode == 'OR', 50, 21) then
            filter_mode = 'OR'
            ScheduleSave(false, 0.5)
        end
    ...
    ImGui.Spacing(ctx)
    if SLM_DrawToggleButton(L("즐겨찾기만 보기", "Favorites only"), 'FavoritesOnly', favorite_only, 120, 23) then
...
local function RenderKeywordEditorPopup()
    ...
    if ImGui.BeginPopupModal(ctx, L("검색어 편집", "Edit Keyword"), nil, ImGui.WindowFlags_AlwaysAutoResize) then
        ...
        local enter_pressed = enter_from_input
            or ImGuiIsKeyPressed(ImGui.Key_Enter)
        local escape_pressed = ImGuiIsKeyPressed(ImGui.Key_Escape)
...
local function DrawAddAndManageTab()
    ...
    local keyword_enter_pressed = ImGuiIsKeyPressed(ImGui.Key_Enter)
    ...
    ImGui.TextColored(ctx, UI_ACCENT_SOFT,
        string.format(L("저장될 태그 · %d개", "Tags to be saved · %d"), #selected_tags_list))
    if #selected_tags_list == 0 then
        ImGui.TextDisabled(ctx, L("현재는 태그 없이 저장됩니다.", "This keyword will currently be saved without tags."))
    else
        local applied_tag_items = {}
        for _, tag_name in ipairs(selected_tags_list) do
            applied_tag_items[#applied_tag_items + 1] = {name=tag_name, color=GetTagColor(tag_name)}
        end
        DrawWrappedTagButtons(applied_tag_items, 'AppliedNewKeywordTag', RemoveSelectedInputTag, false, nil)
        if ImGui.IsItemHovered(ctx) then
            ImGui.SetTooltip(ctx, L("클릭하면 저장 대상에서 제외합니다.", "Click a tag to remove it before saving."))
        end
    end

    ImGui.TextDisabled(ctx, L("사용 가능한 태그 · 클릭하여 선택", "Available tags · Click to select"))
    ...
    local tag_enter_pressed = ImGuiIsKeyPressed(ImGui.Key_Enter)
...
function DrawLanguageButton()
    ...
    if ImGui.BeginPopup(ctx, "LanguagePopup") then
        ...
        if SLM_DrawToggleButton("English", "SelectEnglish", current_language == "en", 110, 23) then
            SetLanguage("en")
            SetStatus(L("언어를 변경했습니다.", "Language changed."), 2.0)
            ImGui.CloseCurrentPopup(ctx)
        end

        if SLM_DrawToggleButton("한국어", "SelectKorean", current_language == "ko", 110, 23) then
            SetLanguage("ko")
            SetStatus(L("언어를 변경했습니다.", "Language changed."), 2.0)
            ImGui.CloseCurrentPopup(ctx)
        end
...
function RunFrame()
    ...
    SLM_ProcessPendingMediaExplorerSend()
    PollMediaExplorerSearchHistory()

    local pushed_font = false
    local pushed_style_vars = 0
    local pushed_style_colors = 0
    local window_begun = false

    local function cleanup_frame()
        if window_begun then
            pcall(ImGui.End, ctx)
            window_begun = false
        end
        if pushed_style_colors > 0 then pcall(ImGui.PopStyleColor, ctx, pushed_style_colors) end
        if pushed_style_vars > 0 then pcall(ImGui.PopStyleVar, ctx, pushed_style_vars) end
        if pushed_font then pcall(ImGui.PopFont, ctx) end
    end

    local ok, frame_result = xpcall(function()
        if ui_font and ImGui.PushFont then
            ImGui.PushFont(ctx, ui_font)
            pushed_font = true
        end

        ImGui.PushStyleVar(ctx, ImGui.StyleVar_WindowPadding, 6, 6); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_WindowRounding, 6.0); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_ChildRounding, 4.0); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_FrameRounding, 4.0); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_FramePadding, 5, 2); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_ItemSpacing, 5, 4); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_ItemInnerSpacing, 3, 3); pushed_style_vars = pushed_style_vars + 1
        ImGui.PushStyleVar(ctx, ImGui.StyleVar_ScrollbarSize, 10); pushed_style_vars = pushed_style_vars + 1

        ImGui.PushStyleColor(ctx, ImGui.Col_WindowBg, 0x171A1FFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_ChildBg, 0x1D2127FF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_FrameBg, 0x252A31FF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_FrameBgHovered, 0x303741FF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_FrameBgActive, 0x39424DFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_Button, 0x2B313AFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_ButtonHovered, 0x39424EFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_ButtonActive, 0x46515FFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_Header, 0x2D3C4CFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_HeaderHovered, 0x395067FF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_HeaderActive, 0x46627DFF); pushed_style_colors = pushed_style_colors + 1
        ImGui.PushStyleColor(ctx, ImGui.Col_Separator, 0x343A43FF); pushed_style_colors = pushed_style_colors + 1

        if not window_state_applied then
            -- Let ReaImGui keep dock attachment; restore only stable floating geometry.
            if window_state.x and window_state.y then
                ImGuiSetNextWindowPosSafe(window_state.x, window_state.y, ImGui.Cond_FirstUseEver)
            end
            ImGuiSetNextWindowSizeSafe(window_state.w, window_state.h, ImGui.Cond_FirstUseEver)
            reaper.SetExtState(EXT_SECTION, 'WindowDockID', '', true)
        ...
        if visible then
            window_begun = true
            if YSL_SLM_ACTION_TOGGLE.pressed() then open = false end
            UpdateDockTransitionState()
            DrawLanguageButton()
            local _, available_height = ImGui.GetContentRegionAvail(ctx)
            local content_height = math.max(200, (tonumber(available_height) or 0) - BOTTOM_NOTICE_H - 4)
            local content_visible = ImGui.BeginChild(ctx, 'MainContent', 0, content_height)
    ...
    cleanup_frame()
    if not ok then error(frame_result, 0) end
    return frame_result
end
...
reaper.atexit(function()
    ...
    -- Never query an ImGui window after its Begin/End frame has finished.
end)
