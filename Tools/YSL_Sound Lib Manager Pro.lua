...
local APP_NAME = 'SLM Pro v1.0.3'
local EXT_KEY_V7 = 'DataV7'
local DATA_VERSION = 11.0
local DEFAULT_WINDOW_W = 760
local DEFAULT_WINDOW_H = 780
local WINDOW_GEOMETRY_VERSION = 3
...
local function RecordMediaExplorerHistory(query)
    ...
    if not entry then
        entry = {query = query, last_used = 0, use_count = 0}
    end
    entry.query = query
...
local function PollMediaExplorerSearchHistory()
    ...
    if query ~= '' and query == media_explorer_monitor.pending_text
        ...
        if RecordMediaExplorerHistory(query) then
...
local function DrawMediaExplorerHistory()
    ...
    ImGui.SameLine(ctx, 0, 8)
    ImGui.TextDisabled(ctx, L('직접 입력한 검색어를 자동 기록', 'Automatically captures typed searches'))
    ImGui.SameLine(ctx)
    local clear_x = ImGui.GetCursorPosX(ctx) + math.max(0, ImGui.GetContentRegionAvail(ctx) - 72)
    ImGui.SetCursorPosX(ctx, clear_x)
    if ImGui.Button(ctx, L('기록 삭제', 'Clear') .. '##ClearMediaHistory', 72, 21) then
        media_explorer_history = {}
        media_explorer_monitor.last_recorded_text = ''
        ScheduleSave(false, 0.1)
    end

    local history_visible = ImGui.BeginChild(ctx, 'MediaExplorerSearchHistory', 0, 112, ImGui.ChildFlags_Border)
    if history_visible then
        ...
        elseif #media_explorer_history == 0 then
            ImGui.TextDisabled(ctx, L('아직 기록이 없습니다. Media Explorer에서 검색하면 여기에 표시됩니다.', 'No history yet. Searches typed in Media Explorer will appear here.'))
        else
            for index = 1, math.min(#media_explorer_history, 12) do
                ...
                local button_width = math.max(160, available - 160)
                if ImGui.Button(ctx, entry.query .. '##MediaHistory' .. tostring(index), button_width, 22) then
                    search_text = entry.query
                    InvalidateVisibleKeywordCache()
                    SetStatus(L('검색 기록을 빠른 검색칸에 불러왔습니다: ', 'Loaded history into Quick Search: ') .. entry.query, 2.5)
                end
                if ImGui.IsItemHovered(ctx) then
                    ImGui.SetTooltip(ctx, L('클릭하면 빠른 검색칸에 불러옵니다.', 'Click to load into Quick Search.'))
                end
                ImGui.SameLine(ctx, 0, 8)
...
local function SendToMediaExplorer(query, used_keywords)
    ...
    -- Native REAPER exposes opening/selecting media, but not the text typed
    -- directly into Media Explorer's search box. Therefore the reliable path
    -- is: enter the query here, then press Enter or this button. The query is
    -- saved immediately and copied for pasting into Media Explorer.
    CopyText(query, nil)
    RecordMediaExplorerHistory(query)
    media_explorer_monitor.last_recorded_text = query
    local _, created, saved = SLM_SaveOrTouchSearchQuery(query)
    ...
    if saved then
        local prefix = created and L('검색어를 자동 저장하고 Media Explorer를 열었습니다. 붙여넣기: ', 'Saved the query and opened Media Explorer. Paste: ')
            or L('검색어 사용 기록을 갱신하고 Media Explorer를 열었습니다. 붙여넣기: ', 'Updated the query history and opened Media Explorer. Paste: ')
        SetStatus(prefix .. query, 5.0)
    else
...
local function DrawKeywordRow(keyword, keyword_index)
    ...
    local action_width = 44 + 44 + 44 + 4 * 2
    local action_gap = 8
...
local function DrawFilterAndSearchControls()
    ...
    local search_input_active = ImGui.IsItemActive(ctx)
    local search_enter_pressed = search_input_active and ImGui.IsKeyPressed(ctx, ImGui.Key_Enter)
    if changed then
        search_text = value
    end
    ...
    if ImGui.IsItemHovered(ctx) then
        ImGui.SetTooltip(ctx, L("현재 검색어를 자동 저장하고 Media Explorer용으로 복사합니다.", "Auto-saves the current query and copies it for Media Explorer."))
    end
    ...
    if search_enter_pressed then SendToMediaExplorer(search_text, {}) end
    ImGui.TextDisabled(ctx, L("이 검색칸에서 Enter 또는 Media Explorer 버튼을 누르면 검색어가 자동 저장됩니다.", "Press Enter here or use Media Explorer to auto-save the query."))
...
function RunFrame()
    ...
    if not window_state_applied then
        if ImGui.SetNextWindowDockID then
            pcall(ImGui.SetNextWindowDockID, ctx, 0, ImGui.Cond_Always or 0)
        end
