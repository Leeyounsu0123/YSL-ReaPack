...
local function DrawWrappedButtons(items, id_prefix, selected_value, on_click)
    ...
    for index, item in ipairs(items) do
        ...
        local selected = selected_value == item.value
        ...
        if SLM_DrawToggleButton(label, id_prefix .. tostring(index), selected, width, 21) then
            on_click(item.value)
        end
    end
end
...
local function MediaExplorerSearchEdit()
    ...
    if valid(media_explorer_monitor.edit) then return media_explorer_monitor.edit end
    ...
    if not valid(hwnd) and toggle > 0 and reaper.OpenMediaExplorer then
...
function SLM_SendToMediaExplorer(query, used_keywords)
    ...
    if not opened and reaper.Main_OnCommand then
    ...
    local edit = MediaExplorerSearchEdit()
    if edit and reaper.JS_Window_SetTitle then
        local ok, result = pcall(reaper.JS_Window_SetTitle, edit, query)
        injected = ok and result ~= false
        if injected and reaper.JS_WindowMessage_Post then
            pcall(reaper.JS_WindowMessage_Post, edit, 'WM_KEYDOWN', 13, 0, 0, 0)
            pcall(reaper.JS_WindowMessage_Post, edit, 'WM_KEYUP', 13, 0, 0, 0)
        end
    end

    if injected then
        SetStatus(L('Media Explorer 검색창에 검색어를 보냈습니다: ', 'Sent the query to Media Explorer: ') .. query, 4.0)
    else
        CopyText(query, nil)
