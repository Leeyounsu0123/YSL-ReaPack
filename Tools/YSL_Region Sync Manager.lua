...
local function load_settings()
  ...
  settings.window_w = clamp(tonumber(get("window_w", tostring(settings.window_w))) or 1180, 700, 4000)
  settings.window_h = clamp(tonumber(get("window_h", tostring(settings.window_h))) or 720, 420, 3000)
...
local function save_settings()
  ...
  settings.window_w = clamp(tonumber(settings.window_w) or 1180, 700, 4000)
  settings.window_h = clamp(tonumber(settings.window_h) or 720, 420, 3000)
...
local function update_window_state(force)
  ...
  if ok_pos and ok_size and w and h and w > 0 and h > 0 then
    ...
    settings.window_w, settings.window_h = w, h
    save_settings()
...
local function run_frame()
  ...
  if not window_state_applied then
    if settings.window_x and settings.window_y and reaper.ImGui_SetNextWindowPos then
      reaper.ImGui_SetNextWindowPos(ctx, settings.window_x, settings.window_y, reaper.ImGui_Cond_Always())
    end
    if reaper.ImGui_SetNextWindowSize then
      reaper.ImGui_SetNextWindowSize(ctx, settings.window_w, settings.window_h, reaper.ImGui_Cond_Always())
    end
