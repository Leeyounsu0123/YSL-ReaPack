...
local DEFAULT_WINDOW_W = 1080
local DEFAULT_WINDOW_H = 780
local MIN_WINDOW_W = 720
local WINDOW_GEOMETRY_VERSION = 5
local REGION_TABLE_DELETE_W = 40
local REGION_TABLE_GO_W = 32
local REGION_TABLE_HIDDEN_W = 42
local REGION_TABLE_ICON_BUTTON_W = 20

-- Re-running the Action List command closes this deferred instance instead of
-- spawning a duplicate window.
if reaper and reaper.set_action_options then
...
YSL_RSM_ACTION_TOGGLE = { bindings = {}, armed = false }

YSL_RSM_ACTION_TOGGLE.key_from_vk = function(vk)
  ...
  local getter = suffix and reaper["ImGui_Key_" .. suffix]
  if type(getter) ~= "function" then return nil end
  local ok, key = pcall(getter)
  return ok and key or nil
end

YSL_RSM_ACTION_TOGGLE.load_bindings = function()
  ...
  for line in file:lines() do
    ...
    if command == token and tonumber(section) == section_id then
      local key = YSL_RSM_ACTION_TOGGLE.key_from_vk(vk)
      if key then
        YSL_RSM_ACTION_TOGGLE.bindings[#YSL_RSM_ACTION_TOGGLE.bindings + 1] = {
...
YSL_RSM_ACTION_TOGGLE.modifiers_match = function(flags, mods)
  local shift = reaper.ImGui_Mod_Shift and reaper.ImGui_Mod_Shift() or 0
  local ctrl = reaper.ImGui_Mod_Ctrl and reaper.ImGui_Mod_Ctrl() or 0
  local alt = reaper.ImGui_Mod_Alt and reaper.ImGui_Mod_Alt() or 0
  local super = reaper.ImGui_Mod_Super and reaper.ImGui_Mod_Super() or 0
...
YSL_RSM_ACTION_TOGGLE.pressed = function()
  if #YSL_RSM_ACTION_TOGGLE.bindings == 0 or
      not reaper.ImGui_IsKeyDown or not reaper.ImGui_IsKeyPressed or
      not reaper.ImGui_GetKeyMods then return false end

  local any_down = false
  for _, binding in ipairs(YSL_RSM_ACTION_TOGGLE.bindings) do
    local ok, down = pcall(reaper.ImGui_IsKeyDown, ctx, binding.key)
    if ok and down then any_down = true; break end
  end

  -- Ignore the key press that launched this instance. The detector becomes
  -- active only after every assigned shortcut key has been released once.
  if not YSL_RSM_ACTION_TOGGLE.armed then
    if not any_down then YSL_RSM_ACTION_TOGGLE.armed = true end
    return false
  end
  ...
  for _, binding in ipairs(YSL_RSM_ACTION_TOGGLE.bindings) do
    if YSL_RSM_ACTION_TOGGLE.modifiers_match(binding.flags, mods) then
      local ok, was_pressed = pcall(reaper.ImGui_IsKeyPressed, ctx, binding.key, false)
...
YSL_RSM_ACTION_TOGGLE.load_bindings()
...
local STRINGS = {
  en = {
    ...
    import_ready = "%d applicable change(s) selected. Loading only creates a preview; click Apply selected to update REAPER.",
    import_no_changes = "The CSV loaded, but no applicable changes are selected. Check the import mode and conflicts.",
    matched_by_number = "Matched by region number and timing because the CSV UID changed.",
    current = "Current",
    incoming = "Incoming",
    action = "Action",
    issue = "Issue",
  ...
  ko = {
    ...
    import_ready = "적용 가능한 변경 %d개가 선택됐습니다. CSV 선택은 미리보기만 생성하므로 REAPER에 반영하려면 선택 변경 적용을 누르세요.",
    import_no_changes = "CSV는 불러왔지만 적용 대상으로 선택된 변경이 없습니다. 가져오기 방식과 충돌 항목을 확인하세요.",
    matched_by_number = "CSV UID가 달라 리전 번호와 시간 구간으로 연결했습니다.",
    current = "현재",
    incoming = "가져올 값",
    action = "작업",
    issue = "문제",
...
local function load_settings()
  ...
  if stored_geometry_version < WINDOW_GEOMETRY_VERSION then
    ...
    settings.window_w = clamp(stored_window_w, MIN_WINDOW_W, 4000)
...
local function delete_project_region_safe(number, trace_label)
  local region_number = math.floor((tonumber(number) or -1) + 0.5)
  local label = tostring(trace_label or ("R" .. tostring(region_number)))
  append_apply_trace(label .. " delete begin")
  local call_ok, result = pcall(reaper.DeleteProjectMarker, 0, region_number, true)
  if not call_ok then
    local message = "DeleteProjectMarker error: " .. tostring(result)
    append_apply_trace(label .. " delete error: " .. message)
    return false, message
  end
  if result ~= true then
    local message = "DeleteProjectMarker rejected R" .. tostring(region_number)
    append_apply_trace(label .. " delete rejected")
    return false, message
  end
  append_apply_trace(label .. " delete complete")
  return true
end
...
local function soft_hide_region_no_undo(row)
  ...
  local ok, delete_error = delete_project_region_safe(row.number, "soft hide R" .. tostring(row.number))
  if not ok then return false, delete_error end
...
local function soft_show_region_no_undo(row)
  ...
  if not live or live.soft_hidden then
    delete_project_region_safe(new_number, "failed restore cleanup R" .. tostring(new_number))
...
local function enumerate_regions_numeric_snapshot()
  -- Build the pre-apply backup only from the rows already captured by the UI.
  -- Re-enumerating project markers or project ExtState immediately before a
  -- delete has caused native REAPER termination on affected portable installs.
  -- Each row retains its original values, so no native API call is needed here.
  local rows = {}
  for _, row in ipairs(state.rows or {}) do
    local original = row.original or row
    rows[#rows + 1] = {
      uid = row.uid or generate_uid(),
      ...
      hidden = row.soft_hidden == true or original.hidden == true,
...
local function run_safe_transaction(label, operation)
  -- Keep marker edits outside an open undo block for host stability. Back up
  -- the already-captured original rows without re-entering native enumeration.
  ...
  append_apply_trace("in-memory original snapshot complete: " .. tostring(#actual_rows))
  ...
  if not success then
    append_apply_trace("apply failed: " .. error_text)
    while #state.report > report_count do table.remove(state.report, 1) end
    add_report("Apply failed", label .. " - " .. error_text .. " | backup: " .. tostring(backup_path), "FAILED")
    set_message(T("apply_failed") .. " " .. error_text .. " | Backup: " .. tostring(backup_path), "error")
    diagnostic_add("ERROR", label .. " failed: " .. error_text .. "; backup=" .. tostring(backup_path))
    state.suppress_change_check_until = reaper.time_precise() + 1.0
    state.pending_refresh_frames = math.max(state.pending_refresh_frames or 0, 2)
    return false
  end
...
local function update_region_row(row, undo_label)
  ...
  if row.hidden ~= original_hidden then
    ...
    elseif original_hidden then
      ...
      local old_guid = row.guid
      local deleted, delete_error = delete_project_region_safe(
        region_number, "restore visible R" .. tostring(region_number))
      if not deleted then
        return false, delete_error or "Could not remove hidden region before restoring it"
      end
      local new_number = reaper.AddProjectMarker2(
...
local function apply_staged_changes()
  ...
  local success = run_safe_transaction("Region Sync Manager: apply staged changes", function()
    ...
    for _, row in ipairs(deletes) do
      local ok, delete_error
      if row.soft_hidden then
        ok = remove_soft_hidden_record(row.uid)
      else
        ok, delete_error = delete_project_region_safe(row.number, "staged R" .. tostring(row.number))
      end
      if not ok then
        error(string.format("Delete failed: R%d %s%s", row.number, row.name,
          delete_error and (" - " .. delete_error) or ""))
      end
      deleted_rows[#deleted_rows + 1] = row
    end

    -- Keep project ExtState writes separate from native marker deletion calls.
    for _, row in ipairs(deleted_rows) do
...
local function create_regions_from_selected_items()
  ...
  local success = run_safe_transaction("Region Sync Manager: create regions from selected items", function()
    ...
    -- Reserve every current project-region name first so newly created names
    -- cannot collide with regions that already exist.
    for _, existing in ipairs(state.rows or {}) do
      reserve_name(tostring(existing.name or ""))
    end
    ...
    for i = 0, count - 1 do
      ...
      local source_name = item_names[i + 1] or "Region"
      local base, sequence_index = split_sequence_suffix(source_name)
      local name = source_name

      if (not sequence_index and (plain_counts[source_name] or 0) > 1) or used_names[source_name] then
        name = next_sequence_name(base)
      else
        reserve_name(name)
      end
...
create_backup_csv = function(rows)
  local _, project_path = reaper.EnumProjects(-1, "")
  local dir = project_path and project_path ~= "" and path_dir(project_path) or ""
  if dir == "" and reaper.GetProjectPathEx then
    dir = tostring(reaper.GetProjectPathEx(0) or "")
  end

  -- Unsaved projects do not have an .rpp directory. Store mandatory safety
  -- backups under the REAPER resource path instead of blocking Apply.
  if dir == "" then
    local root = reaper.GetResourcePath and reaper.GetResourcePath() or "."
    local sep = package.config:sub(1, 1)
    dir = root .. sep .. "Data" .. sep .. "YSL_RegionSyncManager" .. sep .. "Backups"
  end
  if reaper.RecursiveCreateDirectory then pcall(reaper.RecursiveCreateDirectory, dir, 0) end
...
local function build_import_diff()
  ...
  for _, row in ipairs(state.rows) do
    ...
    local number_key = tonumber(row.number)
    if number_key then by_number[math.floor(number_key + 0.5)] = row end
  end
  local matched = {}
  ...
  for _, source_incoming in ipairs(imp.rows) do
    ...
    local current = imp.mode == "name"
      and (current_by_number or current_by_uid)
      or (current_by_uid or current_by_number)
    local timing_matches = current_by_number and
      math.abs((current_by_number.start_sec or 0) - (incoming.start_sec or 0)) <= EPSILON and
      math.abs((current_by_number.end_sec or 0) - (incoming.end_sec or 0)) <= EPSILON
    local trusted_number_fallback = current_by_number and
      (imp.mode == "name" or (not imp.project_mismatch and timing_matches))
    local uid_differs = current and incoming.uid ~= "" and current.uid ~= incoming.uid
    ...
    local status = "added"
    local reason = uid_differs and current == current_by_number and trusted_number_fallback
      and T("matched_by_number") or ""
    ...
    elseif imp.mode ~= "name" and current_by_uid and current_by_number and current_by_uid ~= current_by_number then
      status = "conflict"
      reason = "RegionUID and RegionNumber point to different current regions"
    elseif current then
      if uid_differs and current_by_number == current and not trusted_number_fallback then
        status = "conflict"
...
local function load_import_path(path)
  ...
  for _, diff in ipairs(state.import.diffs) do
    if diff.selected and diff.valid then selected_count = selected_count + 1 end
  end
  if selected_count > 0 then
    set_message(basename(path) .. " · " .. string.format(T("import_ready"), selected_count), "info")
  else
    set_message(basename(path) .. " · " .. T("import_no_changes"), "warning")
  end
  return true
end
...
local function apply_import_diff()
  ...
  local success = run_safe_transaction("Region Sync Manager: apply CSV import", function()
    ...
    for _, row in ipairs(deletes) do
      ...
      else ok, delete_error = delete_project_region_safe(row.number, "import R" .. tostring(row.number)) end
      if not ok then
        error(string.format("Import delete failed: R%d %s - %s", row.number, row.name,
          tostring(delete_error or "unknown error")))
      end
      deleted_rows[#deleted_rows + 1] = row
    end
    for _, row in ipairs(deleted_rows) do
    ...
    for _, d in ipairs(imp.diffs) do
      if d.selected and d.valid and d.status ~= "deleted" and d.status ~= "unchanged" then
        ...
        elseif d.status == "modified" and d.current then
          ...
          if incoming.uid ~= "" and (row.uid == "" or row.uid == incoming.uid) then
            row.uid = incoming.uid
          end
  ...
  if success then
    state.pending_refresh_frames = math.max(state.pending_refresh_frames or 0, 2)
    state.pending_import_rebuild = true
    set_message(T("apply_complete"), "info")
  end
end
...
local function center_cell_item(item_width)
  if not reaper.ImGui_GetContentRegionAvail or not reaper.ImGui_GetCursorPosX or not reaper.ImGui_SetCursorPosX then return end
  local available = select(1, reaper.ImGui_GetContentRegionAvail(ctx)) or 0
  local cursor_x = reaper.ImGui_GetCursorPosX(ctx)
  reaper.ImGui_SetCursorPosX(ctx, cursor_x + math.max(0, (available - (tonumber(item_width) or 0)) * 0.5))
end

local function centered_cell_text(text)
  text = tostring(text or "")
  local width = 0
  if reaper.ImGui_CalcTextSize then width = select(1, reaper.ImGui_CalcTextSize(ctx, text)) or 0 end
  center_cell_item(width)
  reaper.ImGui_Text(ctx, text)
end

local function centered_cell_checkbox(label, value)
  local width = reaper.ImGui_GetFrameHeight and reaper.ImGui_GetFrameHeight(ctx) or 19
  center_cell_item(width)
  return reaper.ImGui_Checkbox(ctx, label, value)
end

local function centered_cell_button(label, width, height)
  width = width or 24
  center_cell_item(width)
  return reaper.ImGui_Button(ctx, label, width, height or 0)
end
...
local function render_regions_tab()
  ...
  reaper.ImGui_SameLine(ctx)
  if reaper.ImGui_Button(ctx, T("apply_changes") .. " (" .. tostring(state.dirty_count) .. ")##apply_regions") then
    state.pending_apply_frames = 2
  end
  reaper.ImGui_SameLine(ctx)
  if reaper.ImGui_Button(ctx, T("discard_changes") .. "##discard_regions") then discard_changes() end
  reaper.ImGui_SameLine(ctx)
  ...
  if reaper.ImGui_BeginTable(ctx, "RegionsTable", 13, flags, -1, -1) then
    ...
    reaper.ImGui_TableSetupColumn(ctx, T("delete"), reaper.ImGui_TableColumnFlags_WidthFixed(), REGION_TABLE_DELETE_W)
    reaper.ImGui_TableSetupColumn(ctx, T("go"), reaper.ImGui_TableColumnFlags_WidthFixed(), REGION_TABLE_GO_W)
    ...
    reaper.ImGui_TableSetupColumn(ctx, T("hidden"), reaper.ImGui_TableColumnFlags_WidthFixed(), REGION_TABLE_HIDDEN_W)
    ...
    for _, row in ipairs(page_rows) do
      ...
      reaper.ImGui_TableNextColumn(ctx)
      centered_cell_checkbox("##sel", row.selected)
      handle_selection_checkbox(row)

      reaper.ImGui_TableNextColumn(ctx)
      centered_cell_text(status_text(row.status))
      ...
      if centered_cell_button(delete_label, REGION_TABLE_ICON_BUTTON_W, 0) then
      ...
      reaper.ImGui_TableNextColumn(ctx)
      if centered_cell_button("▶##go", REGION_TABLE_ICON_BUTTON_W, 0) then reaper.SetEditCurPos(row.start_sec, true, false) end

      reaper.ImGui_TableNextColumn(ctx)
      centered_cell_text("R" .. tostring(row.number))

      reaper.ImGui_TableNextColumn(ctx)
      local ec, ev = centered_cell_checkbox("##enabled", row.enabled)
      ...
      if has_native_region_hidden or has_legacy_region_hidden then
        local hc, hv = centered_cell_checkbox("##hidden", row.hidden)
        if hc then
          row.hidden = hv
          recompute_row_dirty(row)
          recompute_counts()
...
local function render_import_tab()
  ...
  if counts.selected > 0 then
    reaper.ImGui_TextColored(ctx, 0x74D69BFF, string.format(T("import_ready"), counts.selected))
  else
    reaper.ImGui_TextColored(ctx, 0xE6A85CFF, T("import_no_changes"))
  end
...
local region_window_begun = false
...
local function run_frame()
  ...
  if state.pending_refresh_frames and state.pending_refresh_frames > 0 then
    ...
    if state.pending_refresh_frames == 0 then
      local refreshed = refresh_regions(true)
      if refreshed and state.pending_import_rebuild then
        state.pending_import_rebuild = false
        build_import_diff()
      end
    end
  end
  if not window_state_applied then
    local once = reaper.ImGui_Cond_Once and reaper.ImGui_Cond_Once()
      or (reaper.ImGui_Cond_FirstUseEver and reaper.ImGui_Cond_FirstUseEver()) or 0
    -- Never force DockID=0. ReaImGui owns dock attachment; ExtState stores only
    -- the last stable floating rectangle.
    if settings.window_x and settings.window_y and reaper.ImGui_SetNextWindowPos then
      reaper.ImGui_SetNextWindowPos(ctx, settings.window_x, settings.window_y, once)
    end
    if reaper.ImGui_SetNextWindowSize then
      reaper.ImGui_SetNextWindowSize(ctx, settings.window_w, settings.window_h, once)
    end
  ...
  if visible then
    region_window_begun = true
    if YSL_RSM_ACTION_TOGGLE.pressed() then open = false end
    update_dock_transition_state()
    if open ~= false then handle_shortcuts() end
